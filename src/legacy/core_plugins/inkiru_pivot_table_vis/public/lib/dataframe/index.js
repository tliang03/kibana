...
DataFrame.prototype.calculate = function (opts) {
  ...
  this.sortDimension = opts.sortDimension;
...
DataFrame.prototype.getSortValue = function (result) {
  ...
  const sortDimension = this.sortDimension;
  const columns = this.getColumns();
  ...
  const val = getValue(sortCol, result.value, sortDimension);
...
function getValue(col, row, dimensions) {
