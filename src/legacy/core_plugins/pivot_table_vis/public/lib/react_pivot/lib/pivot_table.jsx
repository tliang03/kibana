...
module.exports = createReactClass({
    ...
    return (
      <tbody>
        {rows.map(function(row) {
          return (
            <tr key={row._key + row._level} className={"reactPivot-level-" + row._level}>
              {columns.map(function(col, i) {
                if (i < row._level) return <td key={i} className='reactPivot-indent' />

                return self.renderCell(col, row, i)
              })}
            </tr>
          )
  ...
  renderCell: function(col, row, index) {
    if (col.type === 'dimension') {
      ...
      var text = val
      if (col.template) text = col.template(val, row, col.value, col.digits)
    }
